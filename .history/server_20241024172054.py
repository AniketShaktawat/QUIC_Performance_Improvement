...
async def main():
    ...
    # Wrap the server's transport with LossyTransport
    for transport, protocol in server.transports:
    ...
    # Assuming only one protocol (connection)
    if server.protocols:
        await server.protocols[0].file_sent.wait()
